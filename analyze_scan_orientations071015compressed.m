function out_as=analyze_scan_orientations071015compressed(imagefolder,imageprefix,start_image,end_image,x1,x2,y1,y2,radius,sigma0,AR_z)
tic
splits = 3;
...
if (sIMSCr(1)~=sbp(1))||(sIMSCr(2)~=sbp(2))
    disp('error: size of cropped original image ~= to bandpassed image')
    return
end
%%
...
% for k=1:length(IMSCr(1,1,:))
%    IMSCr(:,:,k)=double(im2bw(IMSCr(:,:,k),thres_val));%thresholding value may change for different frames
% end
...
pksbw = Convol > 0;
% for k=1:size(Convol,3)
%     pksbw(:,:,k)=im2bw(double(Convol(:,:,k)),0.99);%thresholding only works on 2d arrays, may have to adjust
% end
%%

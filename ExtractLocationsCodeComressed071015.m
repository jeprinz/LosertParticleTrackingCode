...
basefolder= 'C:\students\dylan_eric\Compressed_DataRun_071015\';
imageprefix='Image';
savefolder = 'F:\users\dylan_eric\Data Analysis Compressed DataRun_071015\';

sigma0=1; %sharpness of gaussian spherical filter
AR_z=1; % aspect ratio z to x,y

start_image=15; %these are the images corresponding to each z slice in a single frame
end_image=225; %last z slice (top)

x1=50; %these are limits of the region of interest
x2=526;

y1=50; %20;%!!both x2-x1+1 must be odd and same for y
y2=526;

c1=clock;

radius = 9; %measured diameter of 40/2 pixels

largeparts = zeros(1,12);

for i=98:147 %10<--!!!frames
    ...
    result = analyze_scan_orientations071015compressed(imagefolder,imageprefix,start_image,end_image,x1,x2,y1,y2,radius,sigma0,AR_z);
